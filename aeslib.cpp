...
void aeslib::rotWord(unsigned char * tab)
{
...
void aeslib::subWord(unsigned char * tab)
{
	for (int i = 0; i < 4; i++)
		tab[i] = sbox[tab[i] / 16][tab[i] % 16];
}

void aeslib::rCon(unsigned char * tab, int n)
{
	unsigned char c = 1;
	for (int i = 0; i < n - 1; i++)
		c = xTime(c);
...
unsigned char aeslib::xTime(unsigned char b)
{
...
void aeslib::xorWords(unsigned char * tabA, unsigned char * tabB, unsigned char * outTab)
{
	for (int i = 0; i < 4; i++)
		outTab[i] = tabA[i] ^ tabB[i];
}

void aeslib::keyExpansion(unsigned char key[], unsigned char expandedKey[])
{
...
void aeslib::subBytes(unsigned char ** state)
{
	for (int i = 0; i < 4; i++)
		for (int j = 0; j < nb; j++)
...
void aeslib::shiftRows(unsigned char ** state)
{
	unsigned char tmp;
...
unsigned char aeslib::multiplicationBytes(unsigned char a, unsigned char b)
{
...
void aeslib::mixColumns(unsigned char ** state)
{
...
void aeslib::addRoundKey(unsigned char ** state, unsigned char * expandedKey)
{
	for (int i = 0; i < 4; i++)
		for (int j = 0; j < nb; j++)
...
void aeslib::encryptBlock(unsigned char textIn[], unsigned char textOut[], unsigned char key[])
{
...
void aeslib::invSubBytes(unsigned char ** state)
{
	for (int i = 0; i < 4; i++)
		for (int j = 0; j < nb; j++)
...
void aeslib::invMixColumns(unsigned char ** state)
{
	unsigned char tab[4], tab1[4];
	for (int i = 0; i < nb; i++) {
		for (int j = 0; j < 4; j++)
...
void aeslib::invShiftRows(unsigned char ** state)
{
	unsigned char tmp;
	for (int i = 1; i <= 3; i++)
...
void aeslib::decryptBlock(unsigned char textIn[], unsigned char textOut[], unsigned char key[])
{
	unsigned char *expandedKey = new unsigned char[4 * nb*(nr + 1)];
	keyExpansion(key, expandedKey);
	unsigned char **state = new unsigned char *[4];
...
unsigned char *aeslib::encrypt(unsigned char plainText[], int plainTextLen, unsigned  char key[]) {
	unsigned char *outText = new unsigned char[plainTextLen];
	for (int i = 0; i < plainTextLen; i += blockBytesLen) {
		encryptBlock(plainText + i, outText + i, key);
	}
	return outText;
}

unsigned char * aeslib::decrypt(unsigned char encryptedText[], int encryptedTextLen, unsigned char key[])
{
	unsigned char *outText = new unsigned char[encryptedTextLen];
	for (int i = 0; i < encryptedTextLen; i += blockBytesLen) {
